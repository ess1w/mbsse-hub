...
export default function AddPartnerModal({ onClose, onCreated }) {
  const [form, setForm] = useState({
    org_name: '', org_type: 'CSO', focal_person: '', email: '',
    phone: '', sla_signed: false, status: 'Pending', districts: [], notes: '',
  });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.org_type)        { setError('Organisation type is required.'); return; }
    setSaving(true);
    setError('');
    try {
      const created = await organisationsApi.create(form);
      onCreated(created);
    } catch (err) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontSize: 14, fontWeight: 600, color: C.text }}>Add New Partner Organisation</div>
            <div style={{ fontSize: 11, color: C.textSec, marginTop: 2 }}>
              The new organisation will appear in the Partner Directory immediately.
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} style={{ padding: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 14, marginBottom: 14 }}>
            <div>
              ...
              <input value={form.org_name} onChange={e => set('org_name', e.target.value)}
                placeholder="e.g. Save the Children SL" style={FIELD.input} autoFocus />
            </div>
            <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, paddingTop: 20 }}>
              <input type="checkbox" id="sla_signed" checked={form.sla_signed}
                onChange={e => set('sla_signed', e.target.checked)}
                style={{ width: 15, height: 15, cursor: 'pointer' }} />
              <label htmlFor="sla_signed" style={{ fontSize: 12, color: C.text, cursor: 'pointer' }}>
                SLA signed
              </label>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
            ...
            }}>{saving ? 'Saving…' : 'Create organisation'}</button>
          </div>
        </form>
      </div>
    </div>
